...
from .model import AuthType, Config, CredentialType, OAuthConfig, TransportType
...
def load_config(file_name: Union[str, Path]) -> Config:
    ...
    if "oauth" in config_dict:
        ...
        scope_raw = oauth_data.get("scope", [])
        if isinstance(scope_raw, str):
            scope: list[str] = [scope_raw] if scope_raw else []
        elif isinstance(scope_raw, list):
            scope = [str(x) for x in scope_raw if x is not None]
        else:
            scope = []
    ...
    # 创建主配置对象
    try:
        # Backward compatible: accept both `server_port` (preferred) and legacy `server_prot`
        server_port_raw = config_dict.get("server_port")
        if server_port_raw is None:
            server_port_raw = config_dict.get("server_prot")

        debug_raw = config_dict.get("debug", False)
        if isinstance(debug_raw, bool):
            debug = debug_raw
        elif debug_raw is None:
            debug = False
        else:
            debug = parse_bool(str(debug_raw))
        ...
        cfg = Config(
            ...
            debug=debug,
            server_port=server_port_raw if server_port_raw is not None else 8888,
            oauth=oauth_config,
        )
        ...
        env_mapping = [
            ...
            (MCP_SERVER_PORT, "server_port", int, None),
            (MCP_SERVER_DEBUG, "debug", parse_bool, None),
        ]
        ...
        for env_var, attr_name, converter, allowed_values in env_mapping:
            ...
            if env_value_str is not None and env_value_str != "":
                ...
                if allowed_values is not None and value_to_set not in allowed_values:
                    raise ValueError(
                        f"Invalid value '{value_to_set}'. Allowed: {allowed_values}"
                    )
...
def validate_auth_header(
    ...
    server_config: Optional[Config],
    token_store: Optional[dict],
) -> Dict:
    if server_config is None:
        return {"is_valid": False, "error": "Server configuration missing"}
    ...
    try:
        if server_config.auth == AUTH_TYPE_OAUTH:
        ...
        elif server_config.credential == CREDENTIAL_TYPE_TOKEN:
            sts_str = token
            ...
            return {"is_valid": True, "credentials": credentials}
        else:
