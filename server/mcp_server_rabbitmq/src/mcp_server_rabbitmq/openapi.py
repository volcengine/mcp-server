    ...
    def _resolve_node(node: Any) -> Any:
        ...
        if "$ref" in node:
            ...
            if not isinstance(ref_path, str):
                logger.error(
                    f"警告：发现非字符串类型的 $ref 值：{ref_path}，将忽略此 $ref。"
                )
                # 处理字典中 $ref 之外的其他键
                result = {}
            ...
            if ref_path.startswith("#/"):
                ...
                if ref_path in resolving_stack:
                    logger.error(
                        f"警告：检测到循环引用: {ref_path}，将返回 null 或空对象以中断循环。"
                    )
                ...
                target: Any = swagger_dict  # 从根开始查找
                try:
                    for part in parts:
                        ...
                        if isinstance(target, list):
                            ...
                            if (
                                part_unescaped.isdigit()
                                and int(part_unescaped) in target
                            ):
                                target = target[int(part_unescaped)]
                                ...
                                raise KeyError(
                                    f"路径部分 '{part_unescaped}' 在字典中未找到"
                                )
                        else:
                            raise TypeError(
                                f"路径部分 '{part_unescaped}' 无法在非字典/列表类型中查找"
                            )
                ...
                if isinstance(result, dict):
                    ...
                    logger.error(
                        f"警告：$ref '{ref_path}' 指向非对象类型，但同级存在其他属性，这些属性将被忽略。"
                    )
...
def _openapi_to_mcp_tools_and_index(
    openapi_spec: Optional[Dict[str, Any]],
) -> Tuple[List[Tool], Dict[str, List[str]]]:
    """内部转换：返回 tools + tool_name->openapi_path 列表索引（用于运行期反查）"""
    tools: List[Tool] = []
    tool_name_to_paths: Dict[str, List[str]] = {}

    if openapi_spec is None:
        logger.error("警告：OpenAPI 规范为空。")
        return [], {}
    ...
    for path, path_item in paths.items():
        ...
        for method, operation in path_item.items():
            ...
            if operation_id and isinstance(operation_id, str):
                ...
                raw_name = f"{method}_{sanitized_path}"
            ...
            if not isinstance(description, str):
                logger.error(
                    f"警告：操作 {method.upper()} {path} 的描述不是字符串，将使用默认描述。"
                )
                description = f"API 调用: {method.upper()} {path}"

            tool_parameters_dict: Dict[str, Any] = {
                "type": "object",
                "properties": {},
                "required": [],
            }
            ...
            if not isinstance(operation_params, list):
                logger.error(
                    f"警告：操作 {method.upper()} {path} 的 parameters 不是列表，已忽略。"
                )
                operation_params = []

            all_params = [
                p for p in (path_level_params + operation_params) if isinstance(p, dict)
            ]
            ...
            for param_def in reversed(all_params):
                ...
                if (
                    ...
                    or param_name in processed_param_names
                ...
                if isinstance(param_schema, dict):
                    if "$ref_cycle_detected" in param_schema:
                        logger.error(
                            f"警告：参数 '{param_name}' 的 schema 指向一个循环引用，已跳过。"
                        )
                        continue
                    if "description" not in param_schema and param_def.get(
                        "description"
                    ):
                    ...
                    logger.error(
                        f"警告：参数 '{param_name}' 缺少有效的 schema 定义（非字典），已跳过。"
                    )
            ...
            if isinstance(request_body, dict):
                if "$ref_cycle_detected" in request_body:
                    logger.error(
                        f"警告：操作 {method.upper()} {path} 的 requestBody 指向一个循环引用，已跳过。"
                    )
                else:
                    ...
                    if isinstance(content, dict):
                        ...
                        if isinstance(media_type, dict):
                            ...
                            if isinstance(body_schema, dict):
                                if "$ref_cycle_detected" in body_schema:
                                ...
                                elif body_schema.get("type") == "object" and isinstance(
                                    ...
                                    for prop_name, prop_schema in body_schema[
                                        ...
                                        if not isinstance(
                                            ...
                                            logger.error(
                                                f"警告：请求体属性 '{prop_name}' 名称或 schema 无效，已跳过。"
                                            )
                                            continue
            ...
            valid_required = [
                req
                for req in sorted(list(required_params_set))
                if req in param_properties
            ]
            if valid_required:
                tool_parameters_dict["required"] = valid_required
            else:
                tool_parameters_dict.pop("required", None)
            ...
            try:
                ...
                tool_name_to_paths.setdefault(tool_name, []).append(path)
    ...
    return tools, tool_name_to_paths


#  核心转换逻辑（向后兼容）
def openapi_to_mcp_tools(openapi_spec: Optional[Dict[str, Any]]) -> List[Tool]:
    tools, _ = _openapi_to_mcp_tools_and_index(openapi_spec)
    return tools


def openapi_to_mcp_tools_with_index(
    openapi_spec: Optional[Dict[str, Any]],
) -> Tuple[List[Tool], Dict[str, List[str]]]:
    """返回 tools + tool_name->openapi path（可能一对多，需在运行期做唯一性校验）"""
    return _openapi_to_mcp_tools_and_index(openapi_spec)
