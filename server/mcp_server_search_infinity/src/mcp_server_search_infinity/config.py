...
ENV_SEARCH_INFINITY_API_KEY = "SEARCH_INFINITY_API_KEY"
...
def load_config() -> SearchInfinityConfig:
    ...
    try:
        ...
        api_key = os.environ.get(ENV_SEARCH_INFINITY_API_KEY)
        ...
        if not (has_api_key_auth or has_volcengine_auth):
            ...
            if not has_api_key_auth:
                missing_auth.append(ENV_SEARCH_INFINITY_API_KEY)
            ...
            raise ValueError(
                f"Authentication missing. Must provide either {ENV_SEARCH_INFINITY_API_KEY} "
