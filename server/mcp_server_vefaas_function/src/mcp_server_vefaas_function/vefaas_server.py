...
def create_vefaas_application(function_name: Required[str], gateway_name: Required[str], gateway_service_name: Optional[str] = None, 
    ...
    applicationName = function_name + "-app"
                    ...
                    if revision_number: 
                        ...
                        if log_result:
                            ...
                            for logLine in logLines:
                                if "WARN" in logLine or "ERROR" in logLine:
...
- Scenario analysis: When create/deploy veFaaS function, analysis scenario and follow the steps in the following scenario:
    ### Scenario 1: No code in local, generate code from scratch and deploy to a new veFaaS function.
        ...
        5. Upload Code & Install Dependencies: Use `upload_code` to upload the source code. This will also trigger dependency installation if `requirements.txt` or `package.json` is present.
            Before uploading, must do code check:
            - web server must listening on host: 0.0.0.0, port: 8000
            - process must be started by a script, and the script must be executable
            - python runtime's dependency must list in requirements.txt; node runtime's dependency must list in package.json
            - for executable language(e.g. golang), must compile a Linux-compatible binary locally and upload it with all the source code. Make sure there is no compile command in start script
        ...
        9. API Gateway: Use `list_api_gateways` to find a suitable `Running` gateway or `create_api_gateway` if none exist.
        10. Gateway Service: Call `create_api_gateway_service` to create a new service for the function.
