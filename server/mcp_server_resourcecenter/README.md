# resourcecenter MCP Server

| 版本 |         v1.0.0          |
|:--: |:-----------------------:|
| 描述 | 资源中心为火山引擎用户提供跨产品、跨地域的资源统一视图及资源搜索能力，让云上的资源管理更加高效便捷。       |
| 分类 | 工具类 |
| 标签 | 资源管理、资源检索、管理与治理           |
...
## 服务开通链接
https://console.volcengine.com/resourcemanager
...
## 运行 MCP-Server 指南
### 1. 配置文件
`server/mcp_server_resourcecenter/src/mcp_server_resourcecenter/config/config.yaml`
...
- /ABSOLUTE/PATH/TO/PARENT/FOLDER
   - mcp-server-resourcecenter 的代码库目录，例如 /Users/xxx/mcp-server/server/mcp_server_resourcecenter，对应 https://github.com/volcengine/mcp-server/tree/main/server/mcp_server_resourcecenter
...
```json
{
    "mcpServers": {
        "resourcecenter": {
            ...
            "args": [
                "--from",
                "git+https://github.com/volcengine/mcp-server#subdirectory=server/mcp_server_resourcecenter",
                "mcp-server-resourcecenter"
            ],
            "env": {
                "VOLCENGINE_ACCESS_KEY": "your ak",
...
```shell
uv --directory /ABSOLUTE/PATH/TO/PARENT/FOLDER run mcp-server-resourcecenter
```
