...
async def serve() -> None:
    ...
    async def call_tool(
        ...
        try:
            ...
            if error_message is None:
                raise ValueError(f"Error processing mcp-server-cloudcontrol query: {str(apie)}")
            raise ValueError(error_message)
        except Exception as e:
            raise ValueError(f"Error processing mcp-server-cloudcontrol query: {str(e)}")
    ...
    if server_config.transport == TRANSPORT_SSE:
        ...
        config = uvicorn.Config(starlette_app, host="0.0.0.0", port=server_config.server_prot)
        ...
        async def handle_streamable_http(scope: Scope, receive: Receive, send: Send) -> None:
            if server_config.credential == CREDENTIAL_TYPE_TOKEN:
                headers = Headers(scope=scope)
                auth_header = headers.get(AUTHORIZATION_HEADER)
                auth_result = validate_auth_header(auth_header, server_config, {})
                if not auth_result["is_valid"]:
                    body = json.dumps({
