...
class Config:
    ...
    def check(self):
        ...
        if self.transport == "sse" and self.server_prot == 0:
            raise ValueError("sse server_prot can not be 0")

        if self.transport == "StreamableHTTP" and self.server_prot == 0:
            raise ValueError("StreamableHTTP server_prot can not be 0")
