...
VKE（Volcengine Kubernetes Engine）MCP Server 是实现了模型上下文协议（Model Context Protocol）的服务器，可集成到 MCP 客户端（如方舟、Trae、Cursor 等）中，提供与火山引擎 VKE 服务交互的能力，使用自然语言实现集群管理、节点与节点池管理、组件管理等操作，同时支持查询与管理 K8s 集群的资源以及应用 YAML 配置到 K8s 集群。
...
- 当 `ALLOW_WRITE` 为 `true` 时，将支持：
  ...
  - 更新 Addon 版本与配置（通过 `update_addon_version` 与 `update_addon_config` 工具）
- 以下操作不受 `ALLOW_WRITE` 影响，均不支持：
  - **更新**与**删除**集群、节点、节点池
- 默认地域为 `cn-beijing`，如需操作其他地域的资源，可在 prompt 中指定地域，常见的地域包括：
  - `cn-beijing`：华北2（北京）
  - `cn-beijing2`：华北3（北京）
  - `cn-datong`：华北4（大同）
