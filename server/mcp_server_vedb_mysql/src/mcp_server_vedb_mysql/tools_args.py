...
# @mcp_server.tool(
#     description="应用参数模板。触发示例：将参数模板vedbmpt-5j37992t应用到实例vedbm-****，在可维护时间内执行"
# )
def apply_parameter_template(template_id: Annotated[str, Field(examples=['vedbmpt-****'])],
...
# @mcp_server.tool(
#     description="查询实例参数修改历史。触发示例：查询实例vedbm-****的参数修改历史记录"
# )
...
# @mcp_server.tool(
#     description="删除参数模板。触发示例：删除自定义参数模板vedbmpt-****"
# )
...
# @mcp_server.tool(
#     description="修改实例mysqld内核参数（非管控面属性修改）。触发示例：修改实例vedbm-instanceid的内核参数，将innodb_buffer_pool_size设置为2147483648"
# )
...
# @mcp_server.tool(
#     description="查询可修改的参数列表。触发示例：查询实例vedbm-****可修改的参数列表"
# )
