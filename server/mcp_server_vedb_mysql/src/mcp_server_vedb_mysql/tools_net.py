...
@mcp_server.tool(
    description="查询实例绑定的白名单信息。触发示例：查询实例vedbm-r3xq0zdl****当前绑定的所有白名单信息"
)
...
@mcp_server.tool(
    description="查询指定实例连接终端的详细信息。触发示例：查询实例vedbm-r3xq0zdl****的所有连接终端详细信息"
)
...
@mcp_server.tool(
    description="创建连接终端。触发示例：在实例vedbm-r3xq0zdl中创建一个名为自定义读写终端的连接终端，关联节点vedbm-r3xq0zdl-0和vedbm-r3xq0zdl****-1，读写模式为读写"
)
def create_db_endpoint(instance_id: str,
                       nodes: Annotated[str, Field(description='连接终端关联的多个节点ID之间使用英文逗号分隔', examples=['vedbm-2pf2xk5v****-0,vedbm-2pf2xk5v****-1'])],
...
@mcp_server.tool(
    description="为指定的实例创建公网连接地址，即开启实例的公网访问功能。触发示例：为实例vedbm-r3xq0zdl的连接终端vedbm-r3xq0zdl-Custom-50yv创建公网地址"
)
...
@mcp_server.tool(
    description="删除指定实例的公网连接地址，即关闭公网访问功能。触发示例：删除实例vedbm-r3xq0zdl的连接终端vedbm-r3xq0zdl-Custom-50yv的公网地址"
)
...
@mcp_server.tool(
    description="查询当前账号下指定地域内的所有IP白名单。触发示例：查询北京地域下的所有白名单列表，按创建时间排序"
)
def describe_allow_lists(region_id: Annotated[str, Field(examples=['cn-beijing'])],
